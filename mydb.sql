...
--
-- Table structure for table `asset_purchase_orders`
--

CREATE TABLE `asset_purchase_orders` (
  `id` int(11) NOT NULL,
  `apo_number` varchar(20) NOT NULL,
...
--
-- Table structure for table `asset_purchase_order_items`
--

CREATE TABLE `asset_purchase_order_items` (
  `id` int(11) NOT NULL,
  `asset_purchase_order_id` int(11) NOT NULL,
  `asset_id` int(11) NOT NULL,
...
--
-- Table structure for table `CompetitorReport`
--
...
CREATE TABLE `FeedbackReport` (
  ...
  `report_date` date GENERATED ALWAYS AS (cast(`createdAt` as date)) STORED
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE `images_table` (
  `id` int(11) NOT NULL,
  `image_merchandiser_id` int(11) NOT NULL,
  `image_photo` varchar(200) NOT NULL,
  `request_id` int(11) NOT NULL,
  `image_modified_date` varchar(200) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
CREATE TABLE `import_requests` (
  `id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL COMMENT 'Reference to user_client table',
  `request_type` enum('import_request','quote_request') NOT NULL DEFAULT 'import_request',
  `status` enum('pending','in_progress','quoted','approved','rejected','cancelled') NOT NULL DEFAULT 'pending',
  `priority` enum('low','medium','high','urgent') NOT NULL DEFAULT 'medium',
  `vehicle_make` varchar(100) NOT NULL COMMENT 'Car manufacturer/brand',
  `vehicle_model` varchar(100) NOT NULL COMMENT 'Car model',
  `vehicle_year` int(4) NOT NULL COMMENT 'Manufacturing year',
  `vehicle_variant` varchar(100) DEFAULT NULL COMMENT 'Model variant/trim',
  `vehicle_color` varchar(50) DEFAULT NULL COMMENT 'Exterior color',
  `vehicle_interior_color` varchar(50) DEFAULT NULL COMMENT 'Interior color',
  `vehicle_condition` enum('new','used','certified') DEFAULT 'new' COMMENT 'Vehicle condition',
  `vehicle_mileage` int(11) DEFAULT NULL COMMENT 'Mileage for used vehicles',
  `vehicle_transmission` enum('manual','automatic','cvt','semi_automatic') DEFAULT NULL COMMENT 'Transmission type',
  `vehicle_fuel_type` enum('petrol','diesel','hybrid','electric','lpg','cng') DEFAULT NULL COMMENT 'Fuel type',
  `vehicle_engine_size` varchar(20) DEFAULT NULL COMMENT 'Engine displacement',
  `vehicle_drive_type` enum('fwd','rwd','awd','4wd') DEFAULT NULL COMMENT 'Drive type',
  `vehicle_body_type` varchar(50) DEFAULT NULL COMMENT 'Body type (sedan, suv, etc)',
  `vehicle_doors` int(2) DEFAULT NULL COMMENT 'Number of doors',
  `vehicle_seats` int(2) DEFAULT NULL COMMENT 'Number of seats',
  `source_country` varchar(100) DEFAULT NULL COMMENT 'Country where vehicle is located',
  `destination_country` varchar(100) DEFAULT 'Kenya' COMMENT 'Destination country',
  `estimated_price` decimal(12,2) DEFAULT NULL COMMENT 'Estimated price if known',
  `quoted_price` decimal(12,2) DEFAULT NULL COMMENT 'Final quoted price',
  `currency` varchar(3) DEFAULT 'KES' COMMENT 'Price currency',
  `budget_range_min` decimal(12,2) DEFAULT NULL COMMENT 'Minimum budget',
  `budget_range_max` decimal(12,2) DEFAULT NULL COMMENT 'Maximum budget',
  `preferred_delivery_time` enum('asap','1_month','2_months','3_months','flexible') DEFAULT 'flexible' COMMENT 'Preferred delivery timeframe',
  `special_requirements` text DEFAULT NULL COMMENT 'Special requirements or modifications',
  `vin_number` varchar(17) DEFAULT NULL COMMENT 'Vehicle Identification Number if available',
  `registration_number` varchar(50) DEFAULT NULL COMMENT 'Registration number if available',
  `current_location` varchar(255) DEFAULT NULL COMMENT 'Current location of the vehicle',
  `seller_contact` varchar(255) DEFAULT NULL COMMENT 'Seller contact information',
  `seller_website` varchar(255) DEFAULT NULL COMMENT 'Seller website or listing URL',
  `request_notes` text DEFAULT NULL COMMENT 'Additional notes from user',
  `admin_notes` text DEFAULT NULL COMMENT 'Internal admin notes',
  `quoted_by` int(11) DEFAULT NULL COMMENT 'Admin user who provided the quote',
  `quoted_at` timestamp NULL DEFAULT NULL COMMENT 'When the quote was provided',
  `approved_by` int(11) DEFAULT NULL COMMENT 'Admin user who approved the request',
  `approved_at` timestamp NULL DEFAULT NULL COMMENT 'When the request was approved',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp(),
  `updated_at` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Import requests and quote requests for vehicles';

-- --------------------------------------------------------

--
-- Table structure for table `import_request_communications`
--

CREATE TABLE `import_request_communications` (
  `id` int(11) NOT NULL,
  `request_id` int(11) NOT NULL COMMENT 'Reference to import_requests table',
  `sender_id` int(11) NOT NULL COMMENT 'User who sent the message',
  `sender_type` enum('user','admin') NOT NULL COMMENT 'Type of sender',
  `message` text NOT NULL COMMENT 'Message content',
  `message_type` enum('text','quote','status_update','document','system') NOT NULL DEFAULT 'text',
  `is_internal` tinyint(1) DEFAULT 0 COMMENT 'Whether this is an internal admin message',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Communication history for import requests';

-- --------------------------------------------------------

--
-- Table structure for table `import_request_documents`
--

CREATE TABLE `import_request_documents` (
  `id` int(11) NOT NULL,
  `request_id` int(11) NOT NULL COMMENT 'Reference to import_requests table',
  `document_type` enum('vehicle_photo','vehicle_video','inspection_report','registration_document','insurance_document','other') NOT NULL,
  ...
  `description` varchar(255) DEFAULT NULL COMMENT 'Document description',
  `uploaded_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Documents and attachments for import requests';

-- --------------------------------------------------------

--
-- Table structure for table `import_request_status_history`
--

CREATE TABLE `import_request_status_history` (
  `id` int(11) NOT NULL,
  `request_id` int(11) NOT NULL COMMENT 'Reference to import_requests table',
  `old_status` enum('pending','in_progress','quoted','approved','rejected','cancelled') DEFAULT NULL,
  `new_status` enum('pending','in_progress','quoted','approved','rejected','cancelled') NOT NULL,
  `changed_by` int(11) DEFAULT NULL COMMENT 'User who made the change (admin or user)',
  `change_reason` varchar(255) DEFAULT NULL COMMENT 'Reason for status change',
  `notes` text DEFAULT NULL COMMENT 'Additional notes about the change',
  `created_at` timestamp NOT NULL DEFAULT current_timestamp()
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci COMMENT='Status change history for import requests';
...
--
-- Table structure for table `office_out`
--
...
CREATE TABLE `office_out` (
  ...
  `checkin_time` varchar(40) NOT NULL,
  `checkout_time` varchar(40) NOT NULL,
  `longitude` varchar(40) NOT NULL,
  `latitude` varchar(40) NOT NULL,
  `status` int(11) NOT NULL,
  `checkin_address` varchar(200) NOT NULL,
  `checkout_address` varchar(100) NOT NULL
) ENGINE=InnoDB DEFAULT CHARSET=latin1 COLLATE=latin1_swedish_ci;
...
--
-- Indexes for table `asset_purchase_orders`
--
ALTER TABLE `asset_purchase_orders`
  ADD PRIMARY KEY (`id`),
  ADD UNIQUE KEY `apo_number` (`apo_number`),
  ADD KEY `created_by` (`created_by`),
  ADD KEY `idx_apo_number` (`apo_number`),
  ADD KEY `idx_supplier_id` (`supplier_id`),
  ADD KEY `idx_order_date` (`order_date`),
  ADD KEY `idx_status` (`status`);

--
-- Indexes for table `asset_purchase_order_items`
--
ALTER TABLE `asset_purchase_order_items`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_apo_id` (`asset_purchase_order_id`),
  ADD KEY `idx_asset_id` (`asset_id`);
...
--
-- Indexes for table `CompetitorReport`
--
ALTER TABLE `CompetitorReport`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_competitor_report_report_id` (`reportId`),
  ADD KEY `idx_competitor_report_user_id` (`userId`),
  ADD KEY `idx_competitor_report_client_id` (`clientId`),
  ADD KEY `idx_competitor_report_created_at` (`createdAt`),
  ADD KEY `idx_competitor_report_composite` (`reportId`,`createdAt`),
  ADD KEY `idx_competitor_report_plan_date` (`reportId`,`createdAt`),
  ADD KEY `idx_competitor_report_client_date` (`clientId`,`createdAt`),
  ADD KEY `idx_competitor_report_user_date` (`userId`,`createdAt`);
...
ALTER TABLE `expense_payments`
  ADD PRIMARY KEY (`id`),
  ADD KEY `expense_detail_id` (`expense_detail_id`),
  ADD KEY `account_id` (`account_id`),
  ADD KEY `created_by` (`created_by`);
...
ALTER TABLE `FeedbackReport`
  ...
  ADD KEY `idx_feedback_report_report_id` (`reportId`),
  ADD KEY `idx_feedback_report_user_id` (`userId`),
  ADD KEY `idx_feedback_report_client_id` (`clientId`),
  ADD KEY `idx_feedback_report_created_at` (`createdAt`),
  ADD KEY `idx_feedback_report_composite` (`reportId`,`createdAt`);
...
ALTER TABLE `import_requests`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_import_requests_user` (`user_id`),
  ADD KEY `idx_import_requests_status` (`status`),
  ADD KEY `idx_import_requests_type` (`request_type`),
  ADD KEY `idx_import_requests_priority` (`priority`),
  ADD KEY `idx_import_requests_created_at` (`created_at`),
  ADD KEY `idx_import_requests_vehicle` (`vehicle_make`,`vehicle_model`,`vehicle_year`);

--
-- Indexes for table `import_request_communications`
--
ALTER TABLE `import_request_communications`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_import_request_communications_request` (`request_id`),
  ADD KEY `idx_import_request_communications_sender` (`sender_id`),
  ADD KEY `idx_import_request_communications_created_at` (`created_at`);

--
-- Indexes for table `import_request_documents`
--
ALTER TABLE `import_request_documents`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_import_request_documents_request` (`request_id`),
  ADD KEY `idx_import_request_documents_type` (`document_type`);

--
-- Indexes for table `import_request_status_history`
--
ALTER TABLE `import_request_status_history`
  ADD PRIMARY KEY (`id`),
  ADD KEY `idx_import_request_status_history_request` (`request_id`),
  ADD KEY `idx_import_request_status_history_created_at` (`created_at`);
...
ALTER TABLE `leave_requests`
  ...
  ADD KEY `idx_employee_status` (`employee_id`),
  ADD KEY `idx_date_range` (`start_date`,`end_date`),
  ADD KEY `idx_leave_requests_employee_status_date` (`employee_id`,`start_date`),
  ADD KEY `idx_leave_requests_type_status` (`leave_type_id`),
...
--
-- Indexes for table `office_out`
--
ALTER TABLE `office_out`
  ADD PRIMARY KEY (`id`);
...
ALTER TABLE `ProductReport`
  ...
  ADD KEY `idx_product_report_report_id` (`reportId`),
  ADD KEY `idx_product_report_user_id` (`userId`),
  ADD KEY `idx_product_report_client_id` (`clientId`),
  ADD KEY `idx_product_report_created_at` (`createdAt`),
  ADD KEY `idx_product_report_composite` (`reportId`,`createdAt`);
...
ALTER TABLE `VisibilityReport`
  ...
  ADD KEY `idx_visibility_report_report_id` (`reportId`),
  ADD KEY `idx_visibility_report_user_id` (`userId`),
  ADD KEY `idx_visibility_report_client_id` (`clientId`),
  ADD KEY `idx_visibility_report_created_at` (`createdAt`),
  ADD KEY `idx_visibility_report_composite` (`reportId`,`createdAt`);
...
--
-- AUTO_INCREMENT for table `asset_purchase_orders`
--
ALTER TABLE `asset_purchase_orders`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `asset_purchase_order_items`
--
ALTER TABLE `asset_purchase_order_items`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `CompetitorReport`
--
ALTER TABLE `CompetitorReport`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `import_request_communications`
--
ALTER TABLE `import_request_communications`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `import_request_documents`
--
ALTER TABLE `import_request_documents`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;

--
-- AUTO_INCREMENT for table `import_request_status_history`
--
ALTER TABLE `import_request_status_history`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- AUTO_INCREMENT for table `office_out`
--
ALTER TABLE `office_out`
  MODIFY `id` int(11) NOT NULL AUTO_INCREMENT;
...
--
-- Constraints for table `asset_purchase_orders`
--
ALTER TABLE `asset_purchase_orders`
  ADD CONSTRAINT `asset_purchase_orders_ibfk_1` FOREIGN KEY (`supplier_id`) REFERENCES `suppliers` (`id`) ON UPDATE CASCADE,
  ADD CONSTRAINT `asset_purchase_orders_ibfk_2` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`) ON UPDATE CASCADE;

--
-- Constraints for table `asset_purchase_order_items`
--
ALTER TABLE `asset_purchase_order_items`
  ADD CONSTRAINT `asset_purchase_order_items_ibfk_1` FOREIGN KEY (`asset_purchase_order_id`) REFERENCES `asset_purchase_orders` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `asset_purchase_order_items_ibfk_2` FOREIGN KEY (`asset_id`) REFERENCES `my_assets` (`id`) ON UPDATE CASCADE;

--
-- Constraints for table `CompetitorReport`
--
ALTER TABLE `CompetitorReport`
  ADD CONSTRAINT `fk_competitor_report_client` FOREIGN KEY (`clientId`) REFERENCES `Clients` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_competitor_report_user` FOREIGN KEY (`userId`) REFERENCES `SalesRep` (`id`) ON DELETE CASCADE ON UPDATE CASCADE;
...
--
ALTER TABLE `expense_details`
  ADD CONSTRAINT `expense_details_ibfk_1` FOREIGN KEY (`journal_entry_id`) REFERENCES `journal_entries` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `expense_details_ibfk_2` FOREIGN KEY (`supplier_id`) REFERENCES `suppliers` (`id`);

--
-- Constraints for table `expense_items`
--
ALTER TABLE `expense_items`
  ADD CONSTRAINT `expense_items_ibfk_1` FOREIGN KEY (`journal_entry_id`) REFERENCES `journal_entries` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `expense_payments`
--
ALTER TABLE `expense_payments`
  ADD CONSTRAINT `expense_payments_ibfk_1` FOREIGN KEY (`expense_detail_id`) REFERENCES `expense_details` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `expense_payments_ibfk_2` FOREIGN KEY (`account_id`) REFERENCES `chart_of_accounts` (`id`),
  ADD CONSTRAINT `expense_payments_ibfk_3` FOREIGN KEY (`created_by`) REFERENCES `users` (`id`);

--
-- Constraints for table `import_requests`
--
ALTER TABLE `import_requests`
  ADD CONSTRAINT `fk_import_requests_user` FOREIGN KEY (`user_id`) REFERENCES `user_client` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `import_request_communications`
--
ALTER TABLE `import_request_communications`
  ADD CONSTRAINT `fk_import_request_communications_request` FOREIGN KEY (`request_id`) REFERENCES `import_requests` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `import_request_documents`
--
ALTER TABLE `import_request_documents`
  ADD CONSTRAINT `fk_import_request_documents_request` FOREIGN KEY (`request_id`) REFERENCES `import_requests` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `import_request_status_history`
--
ALTER TABLE `import_request_status_history`
  ADD CONSTRAINT `fk_import_request_status_history_request` FOREIGN KEY (`request_id`) REFERENCES `import_requests` (`id`) ON DELETE CASCADE;
...
--
ALTER TABLE `UpliftSale`
  ADD CONSTRAINT `fk_uplift_sale_client` FOREIGN KEY (`clientId`) REFERENCES `Clients` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_uplift_sale_user` FOREIGN KEY (`userId`) REFERENCES `SalesRep` (`id`) ON DELETE CASCADE;

--
-- Constraints for table `UpliftSaleItem`
--
ALTER TABLE `UpliftSaleItem`
  ADD CONSTRAINT `fk_uplift_sale_item_product` FOREIGN KEY (`productId`) REFERENCES `Product` (`id`) ON DELETE CASCADE,
  ADD CONSTRAINT `fk_uplift_sale_item_uplift_sale` FOREIGN KEY (`upliftSaleId`) REFERENCES `UpliftSale` (`id`) ON DELETE CASCADE;
COMMIT;
