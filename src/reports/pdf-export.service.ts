...
export class PdfExportService {
  ...
  async generateVisitsPdf(
    userId: number,
    weekStart?: string,
    format: 'weekly' | 'daily' = 'weekly'
  ): Promise<Buffer> {
    try {
  ...
  private generateVisitsHtml(visitsData: any, weekStart?: string, format: string = 'weekly'): string {
    ...
    // Calculate totals
    const totalVisits = Object.values(visitsData).reduce((total: number, visits: any) => {
      return total + (Array.isArray(visits) ? visits.length : 0);
    }, 0);

    const totalClients = new Set();
    Object.values(visitsData).forEach((visits: any) => {
      if (Array.isArray(visits)) {
        visits.forEach((visit: any) => {
          if (visit.clientId) totalClients.add(visit.clientId);
        });
      }
    });
...
<body>
    ...
    <div class="summary">
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-number">${(totalVisits as number) > 0 ? ((totalVisits as number) / (Object.keys(visitsData).length as number)).toFixed(1) : '0'}</div>
                <div class="stat-label">Avg Visits/Day</div>
            </div>
        </div>
    </div>

    ${Object.entries(visitsData).map(([dateKey, visits]: [string, any], index) => {
